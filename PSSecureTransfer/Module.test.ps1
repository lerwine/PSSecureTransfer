...
ConvertTo-EncryptedFile -Key:$TempKeyName -Path:$OriginalPath -Destination:$EncryptedPath;

$DecryptedPath = [System.IO.Path]::GetTempFileName();

ConvertFrom-EncryptedFile -Path:$EncryptedPath -Destination:$DecryptedPath;
